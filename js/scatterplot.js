...
let yScale3 = d3.scaleLinear() //sets the scale to be linear
            .domain([0,maxY3]) //represents the inputs for the function, so 0 to max score value
...
let xScale3 = d3.scaleTime() //constructs new band scale
            .domain(d3.range(data3.length)) //specifies input (array of values)
...
svg2.append("g")//sets the placeholder svg "g"
    ...
    .call(d3.axisBottom(xScale1) // gives x Axis scale
            .tickFormat(i => data1[i].name)) // sets the tick marker names as "names" from data1 for each tick
...
const mousemove2 = function(event, d) {
  tooltip2.style("left", (event.x)+"px") // sets the tooltip to be based off of the x and y pixel position of mouse 
          .style("top", (event.y + yTooltipOffset) +"px"); 
}
